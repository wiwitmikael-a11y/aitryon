...
const ResultDisplay: React.FC<ResultDisplayProps> = ({ generatedImage, isLoading, error }) => {
  ...
  const renderContent = () => {
    ...
    if (error) {
      return (
        <div className="flex flex-col items-center justify-center h-full text-center bg-red-900/20 border border-red-500/30 p-4 rounded-lg">
           <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
             <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
           </svg>
          <p className="text-red-300 mt-4 font-semibold">An Error Occurred</p>
          <p className="text-slate-400 mt-2 text-sm break-words">{error}</p>
        </div>
      );
    }
  ...
  return (
    <div className="w-full h-full min-h-[400px] lg:min-h-full flex items-center justify-center bg-slate-800/50 rounded-lg p-4 border border-slate-700/50">
        {renderContent()}
    </div>
  );
};
